...
本機（台灣 IP、日盤時段）實測同一組呼叫可正常解析，故問題不在解析邏輯
本身，而在正式環境該時點的回應內容。要查清楚就得先把它記下來。
"""
import httpx
import pytest
...
NIGHT_OK = [{"SymbolID": "TXFH6-M", "CLastPrice": "46389.00"}]
DAY_OK = [{"SymbolID": "TXFH6-F", "CLastPrice": "46170.00"}]


def _patch_quotes(monkeypatch, night, day):
    def fake(market_type: str):
        return night if market_type == "1" else day

    monkeypatch.setattr(market_context, "_taifex_quotes", fake)


def test_successful_fetch_logs_nothing(monkeypatch, caplog):
    _patch_quotes(monkeypatch, NIGHT_OK, DAY_OK)
...
@pytest.mark.parametrize(
    "night,day,missing",
    [
        ([{"SymbolID": "TXFH6-M", "CLastPrice": ""}], DAY_OK, "夜盤"),
        (NIGHT_OK, [{"SymbolID": "TXFH6-F", "CLastPrice": ""}], "日盤"),
        ([], [], "夜盤"),
    ],
)
def test_unparsable_response_is_logged_with_evidence(
    monkeypatch, caplog, night, day, missing
):
    """解析不出來時要記下實際看到的東西，否則無從判斷是收盤空窗還是格式變更。"""
    _patch_quotes(monkeypatch, night, day)

    with caplog.at_level("WARNING", logger="app.services.market_context"):
        assert market_context._fetch_tw_night_futures() is None

    assert caplog.text, "解析失敗卻完全沒有 log——這正是查不出原因的癥結"
    # 光說「失敗」沒用，要能看出是哪一邊、當時的值長怎樣
    assert "夜盤" in caplog.text and "日盤" in caplog.text
...
def test_digest_is_bounded(monkeypatch, caplog):
    ...
    _patch_quotes(
        ...
        [{"SymbolID": f"TXF{i}-F", "CLastPrice": ""} for i in range(50)],
    )
